...
export default function EditSegmentPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await api.segments.get(segmentId);
        if (res.success) {
          const seg = res.data;
          setName(seg.name);
          ...
          if (rules) {
            ...
            setConditions(rules.conditions?.map((c: any) => ({ field: c.field, op: c.op, value: String(c.value) })) || [emptyCondition()]);
          }
          
          // Load initial preview
          setPreview({
            count: seg.customerCount,
            samples: [] // Full samples won't be in the get response, but they can hit "Preview" again.
          });
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api.segments.update(segmentId, { name, description: description || undefined, rules });
      router.push('/segments');
    } catch (err) {
      alert((err as Error).message);
      setSaving(false);
    ...
    <div className="max-w-3xl space-y-6 animate-slide-up">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Filter className="h-6 w-6 text-purple-400" /> Edit Segment
        </h1>
        <p className="text-sm text-slate-400 mt-1">Modify your audience using AI or manual rules.</p>
      </div>
